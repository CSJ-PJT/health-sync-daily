...
begin
  ...
  where link_token = supplied_link_token
    ...
    and product_key = requested_product_key
  ...
  return jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'productKey', link_row.product_key,
    ...
    coalesce(
      (
        ...
        where mission.link_id = link_row.id
          and mission.product_key = requested_product_key
      ),
      '[]'::jsonb
    ),
    'rewards',
    coalesce(
      (
        ...
        where reward.link_id = link_row.id
          and reward.product_key = requested_product_key
      ),
      '[]'::jsonb
    )
  );
...
begin
  ...
  insert into public.life_sim_player_states (link_id, slot, state, product_key, updated_at)
  values (link_row.id, requested_slot, requested_state, requested_product_key, now())
