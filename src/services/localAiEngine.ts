...
export function generateLocalAiReply(message: string, context?: AiContext) {
  ...
  if (hasAny(normalized, ["수면", "잠", "휴식"])) {
    return "수면과 회복은 훈련의 질을 크게 좌우합니다. 최근 수면 시간과 피로감도 함께 기록해 보세요.";
  }
  ...
  if (hasAny(normalized, ["영양", "칼로리", "단백질", "식단"])) {
    return "영양은 회복과 훈련 적응에 직접 연결됩니다. 운동 강도에 맞춰 단백질과 수분 보충을 먼저 챙겨 보세요.";
  }
  if (hasAny(normalized, ["챌린지", "배지", "게임"])) {
    return "챌린지와 게임 기록은 꾸준함을 만드는 데 좋습니다. 이번 주 목표를 작게 잡고 계속 이어가 보세요.";
  }
  if (context?.roomType === "group") {
    return `${context.roomName || "그룹"} 기준으로 보면 각자 기록과 회복 상태를 같이 비교해 보는 것이 좋겠습니다.`;
  }
  if (context?.participantNames?.length) {
    return `${context.participantNames[0]}와의 대화라면 오늘 기록이나 컨디션을 짧게 주고받는 것부터 시작해 보세요.`;
  }
  return "좋습니다. 오늘 기록, 컨디션, 목표 중 하나를 먼저 정해서 이야기하면 더 구체적으로 정리해 드릴 수 있습니다.";
}
