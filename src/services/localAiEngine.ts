...
export function generateLocalAiReply(message: string, context?: AiContext) {
  ...
  if (hasAny(normalized, ["수면", "잠", "휴식"])) {
    return "수면과 회복은 훈련 품질을 크게 좌우합니다. 최근 수면 시간과 피로감도 함께 기록해 보세요.";
  }
  if (hasAny(normalized, ["심박", "heart"])) {
    return "심박은 강도 판단에 좋습니다. 평소보다 높다면 강도를 조금 낮추고 회복 상태를 먼저 확인해 보세요.";
  }
  if (hasAny(normalized, ["영양", "칼로리", "단백질", "식단"])) {
    return "영양은 회복과 훈련 적응에 직접 연결됩니다. 운동 강도에 맞춰 수분과 단백질 보충부터 챙겨 보세요.";
  }
  if (hasAny(normalized, ["챌린지", "배지", "게임"])) {
    return "챌린지와 게임 기록은 꾸준함을 만드는 데 도움이 됩니다. 이번 주 목표를 작게 잡고 이어가 보세요.";
  }
