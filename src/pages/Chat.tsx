...
const Chat = () => {
  ...
  const renderRoomsList = () => {
    const content = (
      <div className="space-y-3">
        {rooms.length === 0 ? (
          <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">대화 목록이 없습니다.</div>
        ) : (
          rooms.map((room) => (
            <button
              key={room.id}
      ...
      <div className="mx-auto max-w-6xl space-y-4 p-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2" data-no-swipe="true">
            ...
            <Button variant={listTab === "friends" && showLists ? "default" : "outline"} size="icon" onClick={() => { setShowLists(true); setListTab("friends"); }} aria-label="친구 목록">
              <ContactRound className="h-4 w-4" />
            </Button>
            <Button variant={listTab === "rooms" && showLists ? "default" : "outline"} size="icon" onClick={() => { setShowLists(true); setListTab("rooms"); }} aria-label="대화 목록">
                  ...
                  <div className="space-y-4">
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <Button onClick={handleManualAdd} className="w-full">친구 추가</Button>
                      </CardContent>
                    </Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {actionTarget.kind === "friend" ? (
                  <Button variant="outline" onClick={handleStartChatFromFriend}>대화 시작</Button>
                ) : null}
