  ...
  switch (questId) {
    case "first-harvest":
      return state.storyFlags.harvestedFirstCrop
        ? { ko: "첫 수확을 완료했습니다.", en: "First harvest completed." }
    ...
    case "mine-recon":
      return state.storyFlags.enteredMine
        ...
        : { ko: "마을 북쪽 통로를 통해 광산 유적에 들어가 보세요.", en: "Enter the mine ruins through the northern route." };
    case "repair-lantern":
      return state.storyFlags.repairedLantern
        ? { ko: "정화 등불 복구를 마쳤습니다.", en: "Purity lantern repaired." }
        : { ko: "광산에서 자원을 모아 정화 등불을 제작하고 정비공에게 가져가세요.", en: "Craft a purity lantern and bring it to the mechanic." };
    case "restore-bridge":
      return state.storyFlags.restoredBridge
        ? { ko: "붕괴한 다리를 복원했습니다.", en: "The collapsed bridge is restored." }
        : { ko: "복구 키트를 제작해 농장 북쪽 다리 표식 앞에서 사용하세요.", en: "Craft a repair kit and use it at the farm bridge marker." };
  }
}
...
export function getActiveQuestSummary(state: LifeSimState) {
  ...
  if (!active) {
    return {
      title: t({ ko: "모든 현재 퀘스트 완료", en: "Current quests complete" }),
      objective: t({ ko: "다음 업데이트에서 이어질 긴 새벽의 새 장을 기다리세요.", en: "Wait for the next chapter of the Longest Dawn." }),
    };
  }
