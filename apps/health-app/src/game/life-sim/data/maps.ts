...
function buildFarmTiles(): LifeSimTile[] {
  ...
  return tiles.map((entry) => {
    ...
    if (entry.x === 13 && entry.y === 3) {
      return { ...entry, bed: true, signText: { ko: "이 자리에서 하루를 마무리할 수 있습니다.", en: "You can sleep here." } };
    }
    return entry;
  });
}
  ...
  return tiles.map((entry) => {
    ...
    if (entry.x === 9 && entry.y === 3) {
      return {
        ...entry,
        signText: {
          ko: "광장 아래에는 잊힌 관로와 봉인된 기록실이 잠들어 있다는 소문이 있습니다.",
          en: "Rumor says sealed infrastructure sleeps beneath the square.",
        },
      };
    }
    return entry;
  ...
  return tiles.map((entry) => {
    ...
    if (entry.x === 12 && entry.y === 5) {
      return {
        ...entry,
        signText: {
          ko: "부서진 문양 아래에서 오래된 그림자 행정 구조의 흔적이 희미하게 반짝입니다.",
...
export const lifeSimMaps: Record<string, LifeSimMapDefinition> = {
  farm: {
    id: "farm",
    name: { ko: "복원 농장", en: "Recovery Farm" },
    width: 18,
    height: 14,
    tiles: buildFarmTiles(),
    ambientHint: {
      ko: "손상된 정착지 가장자리에 남은 농장. 새벽의 바람은 아직 이 땅이 회복될 수 있다고 속삭입니다.",
      en: "A farm on the edge of a damaged settlement. The dawn wind still whispers that the soil can heal.",
    },
  },
  village: {
    id: "village",
    ...
    ambientHint: {
      ko: "평화로운 광장 아래에는 잊힌 체계의 기록과 봉인된 설비가 잠들어 있습니다.",
      en: "Beneath the peaceful square lie records of a forgotten system.",
    },
  },
  mine: {
    id: "mine",
    name: { ko: "정화 광맥", en: "Purifier Ruins" },
    width: 18,
    height: 14,
    tiles: buildMineTiles(),
    ambientHint: {
      ko: "붕괴된 통로와 오래된 정화 설비가 뒤엉킨 장소. 그림자의 흔적이 아직 완전히 사라지지 않았습니다.",
      en: "Collapsed passages and purifier conduits twist together here, marked by old shadows.",
    },
  },
};
