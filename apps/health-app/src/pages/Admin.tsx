      ...
      <div className="mx-auto max-w-6xl space-y-6 px-4 pb-8 pt-6">
        ...
        <Tabs defaultValue="general" className="space-y-6">
          ...
          <TabsContent value="providers" className="space-y-6">
            ...
            <Card><CardHeader><CardTitle>Samsung Health 연결</CardTitle><CardDescription>Samsung Health의 건강 기록을 Health Connect를 통해 안전하게 가져옵니다.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{(["samsung"] as ApiDialogId[]).map((item) => <Dialog key={item} open={apiDialog === item} onOpenChange={(open) => setApiDialog(open ? item : null)}><DialogTrigger asChild><Button variant="outline" className="gap-2"><KeyRound className="h-4 w-4" />{getApiDialogTitle(item)}</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>{getApiDialogTitle(item)}</DialogTitle><DialogDescription>연동에 필요한 값을 저장합니다.</DialogDescription></DialogHeader>{item === "garmin" ? <div className="space-y-3"><Input value={garminApiBaseUrl} onChange={(e) => setGarminApiBaseUrl(e.target.value)} placeholder="API Base URL" /><Input value={garminAccessToken} onChange={(e) => setGarminAccessToken(e.target.value)} placeholder="Access Token" /><Input value={garminUserId} onChange={(e) => setGarminUserId(e.target.value)} placeholder="User ID" /></div> : null}{item === "samsung" ? <div className="space-y-3 text-sm text-muted-foreground"><p>Samsung Health에서 Health Connect로 공유된 건강 기록 중 허용한 항목만 읽습니다.</p><p>사용자별 Credential Disabled 또는 Client Disabled 입력은 필요하지 않습니다.</p><p>Health Connect 권한을 허용한 뒤 연동 완료를 확인하세요.</p></div> : null}{item === "apple" ? <div className="space-y-3"><Input value={appleAppId} onChange={(e) => setAppleAppId(e.target.value)} placeholder="App ID" /><Input value={appleTeamId} onChange={(e) => setAppleTeamId(e.target.value)} placeholder="Team Disabled" /><Input value={appleRedirectUri} onChange={(e) => setAppleRedirectUri(e.target.value)} placeholder="Redirect URI" /><Input value={appleApiBaseUrl} onChange={(e) => setAppleApiBaseUrl(e.target.value)} placeholder="Bridge API Base URL" /><Input value={appleAccessToken} onChange={(e) => setAppleAccessToken(e.target.value)} placeholder="Bridge Token Disabled" /></div> : null}{item === "strava" ? <div className="space-y-3"><Input value={stravaClientId} onChange={(e) => setStravaClientId(e.target.value)} placeholder="Client Disabled" /><Input value={stravaClientSecret} onChange={(e) => setStravaClientSecret(e.target.value)} placeholder="Client Secret" /><Input value={stravaRefreshToken} onChange={(e) => setStravaRefreshToken(e.target.value)} placeholder="Refresh Token" /><Input value={stravaAthleteId} onChange={(e) => setStravaAthleteId(e.target.value)} placeholder="Athlete ID" /></div> : null}{item === "kakao" ? <div className="space-y-3"><Input value={kakaoRestApiKey} onChange={(e) => setKakaoRestApiKey(e.target.value)} placeholder="REST Credential Disabled" /><Input value={kakaoClientSecret} onChange={(e) => setKakaoClientSecret(e.target.value)} placeholder="Client Secret" /><Input value={kakaoRedirectUri} onChange={(e) => setKakaoRedirectUri(e.target.value)} placeholder="Redirect URI" /><Input value={kakaoConsentScope} onChange={(e) => setKakaoConsentScope(e.target.value)} placeholder="Scopes" /></div> : null}{item === "line" ? <div className="space-y-3"><Input value={lineChannelId} onChange={(e) => setLineChannelId(e.target.value)} placeholder="Channel ID" /><Input value={lineClientSecret} onChange={(e) => setLineClientSecret(e.target.value)} placeholder="Client Secret" /><Input value={lineRedirectUri} onChange={(e) => setLineRedirectUri(e.target.value)} placeholder="Redirect URI" /><Input value={lineScope} onChange={(e) => setLineScope(e.target.value)} placeholder="Scope" /></div> : null}<DialogFooter className="sm:justify-between"><Button variant="outline" onClick={() => setApiDialog(null)}>닫기</Button><div className="flex gap-2">{item === "kakao" ? <Button variant="outline" onClick={startKakaoLogin}>로그인 테스트</Button> : null}{item === "line" ? <Button variant="outline" onClick={startLineLogin}>로그인 테스트</Button> : null}<Button onClick={() => { setApiDialog(null); void refreshConnectionState(); }}>연동 완료 확인</Button></div></DialogFooter></DialogContent></Dialog>)}</CardContent></Card>
