...
import {
  getSamsungLastSyncAt,
  setSamsungLastSyncAt,
} from "@/providers/samsung/services/samsungConnectionStore";
import { samsungProvider } from "@/providers/samsung/services/samsungProvider";
import {
  getStravaProviderConfig,
...
type ApiDialogId = "samsung" | "apple" | "strava" | "kakao" | "line";

const providers: ProviderId[] = ["samsung"];
...
function getRecentDateStrings(days: number) {
  return Array.from({ length: days }, (_, index) =>
    getDateStringWithOffset(days - index - 1),
  );
}
...
function getApiDialogTitle(_type: ApiDialogId) {
  return "Samsung Health 연결";
}
...
function Admin() {
  ...
  const [providerValidationSummary, setProviderValidationSummary] =
    useState<ProviderValidationSummary | null>(null);
  ...
  const [testingProviderId, setTestingProviderId] = useState<ProviderId | null>(
    null,
  );
  const [backfillingProviderId, setBackfillingProviderId] =
  ...
  const [samsungApiKey, setSamsungApiKey] = useState(
    localStorage.getItem("samsung_health_api_key") || "",
  );
  const [samsungClientId, setSamsungClientId] = useState(
    localStorage.getItem("samsung_health_client_id") || "",
  );
  ...
  const [kakaoRedirectUri, setKakaoRedirectUri] = useState(
    "https://rhhealthcare.app/auth/kakao/callback",
  );
  const [kakaoConsentScope, setKakaoConsentScope] = useState(
    "profile_nickname,account_email,name,phone_number",
  );
  const [lineChannelId, setLineChannelId] = useState("");
  const [lineClientSecret, setLineClientSecret] = useState("");
  const [lineRedirectUri, setLineRedirectUri] = useState(
    "https://rhhealthcare.app/auth/line/callback",
  );
  ...
  const [displayRecordType, setDisplayRecordTypeState] = useState<RecordType>(
    getDisplayedRecordType(),
  );
  const [backgroundTone, setBackgroundTone] = useState(
    localStorage.getItem("app_background_hsl") || "",
  );

  const featuredRecord = useMemo(
    () =>
      verifiedRecords.find(
        (record) => record.type === displayRecordType && record.certified,
      ) || null,
  ...
  useEffect(() => {
    ...
    setKakaoRedirectUri(
      kakaoConfig.redirectUri || "https://rhhealthcare.app/auth/kakao/callback",
    );
    setKakaoConsentScope(
      kakaoConfig.consentScope ||
        "profile_nickname,account_email,name,phone_number",
    );
    setLineChannelId(lineConfig.channelId);
    setLineClientSecret(lineConfig.clientSecret);
    setLineRedirectUri(
      lineConfig.redirectUri || "https://rhhealthcare.app/auth/line/callback",
    );
  ...
  async function refreshConnectionState() {
    ...
    try {
      ...
      if (status.lastSyncAt)
        ...
        setProviderLastSync(
          samsungSync ? new Date(samsungSync).toLocaleString("ko-KR") : "",
        );
      }
    } catch (error) {
  ...
  async function handleRecordFileChange(
  ...
  function saveRecord() {
    ...
    toast({
      title: "인증 기록을 저장했습니다",
      description: buildRecordTag(next[0]) || officialTime.trim(),
    });
  }
  ...
  async function testProviderFetch(providerId: ProviderId) {
    ...
    try {
      ...
      await createTransferLog(
        "provider_test",
        "success",
        `${provider.displayName} 실데이터 테스트 성공`,
      );
  ...
  async function backfillProviderData(providerId: ProviderId, days: number) {
    ...
    try {
      ...
      for (const date of dateStrings) {
        try {
          ...
          await createTransferLog(
            ...
            "error",
            `${provider.displayName} ${date} 실패 - ${reason}`,
          );
        }
      }
      ...
      await createTransferLog(
        "provider_backfill",
        "error",
        error instanceof Error ? error.message : "provider backfill failed",
      );
      toast({
  ...
  async function handleDeletionRequest() {
    const success = await submitServerDeletionRequest(
      "사용자가 서버 데이터 삭제 요청을 제출했습니다.",
    );
    toast({
      title: success
        ? "삭제 요청을 제출했습니다"
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-4 pb-8 pt-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            연동, 기록, 장비, 데이터 권리와 테마를 한 곳에서 관리합니다.
          </p>
        </div>
        ...
        <Tabs defaultValue="general" className="space-y-6">
          ...
          <TabsContent value="general" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                    {providers.map((providerId) => (
                      <Button
                        key={providerId}
                        variant={
                          activeProvider === providerId ? "default" : "outline"
                        }
                ...
                <div className="flex items-center justify-between rounded-xl border p-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      실연동이 준비되지 않아도 모든 화면을 mock 데이터로
                      채웁니다.
                    </p>
                  </div>
                  <Switch
                    id="mock-mode"
                    checked={mockHealthDataEnabled}
                ...
                <div className="rounded-xl border p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <div className="text-right text-xs text-muted-foreground">
                      ...
                      <div>
                        예상 잔여 토큰: {remainingTokens.toLocaleString()}
                      </div>
                    </div>
                  </div>
                </div>
                <Button
                  onClick={() =>
                    navigate("/account-settings", { state: { from: "/admin" } })
                  }
          ...
          <TabsContent value="providers" className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  {providerAuthExpiresAt ? (
                    <div className="text-sm text-muted-foreground">
                      인증 만료: {formatDateTime(providerAuthExpiresAt)}
                    </div>
                  ) : null}
                  {providerMessage ? (
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={syncTime}
                      onChange={(event) =>
                        handleSyncTimeChange(event.target.value)
                      }
                    />
                  </div>
                  <Button
                    onClick={() => void syncHealthData()}
                    disabled={isSyncing}
                    className="w-full gap-2"
                  >
                    <RefreshCw
            ...
            <Card>
              <CardHeader>
                <CardTitle>Samsung Health 연결</CardTitle>
                <CardDescription>
                  Samsung Health의 건강 기록을 Health Connect를 통해 안전하게
                  가져옵니다.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {(["samsung"] as ApiDialogId[]).map((item) => (
                  <Dialog
                    key={item}
                    open={apiDialog === item}
                    onOpenChange={(open) => setApiDialog(open ? item : null)}
                  >
                    <DialogTrigger asChild>
                      <Button variant="outline" className="gap-2">
                        <KeyRound className="h-4 w-4" />
                        {getApiDialogTitle(item)}
                      </Button>
                    </DialogTrigger>
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>{getApiDialogTitle(item)}</DialogTitle>
                        <DialogDescription>
                          연동에 필요한 값을 저장합니다.
                        </DialogDescription>
                      </DialogHeader>
                      {item === "samsung" ? (
                        <div className="space-y-3 text-sm text-muted-foreground">
                          <p>
                            Samsung Health에서 Health Connect로 공유된 건강 기록
                            중 허용한 항목만 읽습니다.
                          </p>
                          <p>
                            사용자별 Credential Disabled 또는 Client Disabled
                            입력은 필요하지 않습니다.
                          </p>
                          <p>
                            Health Connect 권한을 허용한 뒤 연동 완료를
                            확인하세요.
                          </p>
                        </div>
                      ) : null}
                      {item === "apple" ? (
                        <div className="space-y-3">
                          ...
                          <Input
                            ...
                            placeholder="Team Disabled"
                          />
                          <Input
                            value={appleRedirectUri}
                            onChange={(e) =>
                              setAppleRedirectUri(e.target.value)
                            }
                            placeholder="Redirect URI"
                          />
                          <Input
                            value={appleApiBaseUrl}
                            onChange={(e) => setAppleApiBaseUrl(e.target.value)}
                            placeholder="Bridge API Base URL"
                          />
                          <Input
                            value={appleAccessToken}
                            onChange={(e) =>
                              setAppleAccessToken(e.target.value)
                            }
                            placeholder="Bridge Token Disabled"
                          />
                        </div>
                      ) : null}
                      {item === "strava" ? (
                        <div className="space-y-3">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setStravaRefreshToken(e.target.value)
                            }
                      ...
                      {item === "kakao" ? (
                        <div className="space-y-3">
                          <Input
                            ...
                            placeholder="REST Credential Disabled"
                          />
                          <Input
                            value={kakaoClientSecret}
                            onChange={(e) =>
                              setKakaoClientSecret(e.target.value)
                            }
                            placeholder="Client Secret"
                          />
                          <Input
                            ...
                            onChange={(e) =>
                              setKakaoRedirectUri(e.target.value)
                            }
                            placeholder="Redirect URI"
                          />
                          <Input
                            value={kakaoConsentScope}
                            onChange={(e) =>
                              setKakaoConsentScope(e.target.value)
                            }
                            placeholder="Scopes"
                          />
                        </div>
                      ...
                      {item === "line" ? (
                        <div className="space-y-3">
                          ...
                          />
                          <Input
                            value={lineClientSecret}
                            onChange={(e) =>
                              setLineClientSecret(e.target.value)
                            }
                            placeholder="Client Secret"
                          />
                          <Input
                            value={lineRedirectUri}
                            onChange={(e) => setLineRedirectUri(e.target.value)}
                            placeholder="Redirect URI"
                          />
                          <Input
                            value={lineScope}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  공급자별 최근 7일 또는 30일 데이터를 한 번에 적재합니다.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {providers.map((providerId) => (
                  <div
                    key={`${providerId}-backfill`}
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {backfillingProviderId === providerId
                          ? "적재 중..."
                          : "최근 7일"}
                      </Button>
                      <Button
                        ...
                        disabled={backfillingProviderId !== null}
                        onClick={() =>
                          void backfillProviderData(providerId, 30)
                        }
                      >
                        {backfillingProviderId === providerId
                          ? "적재 중..."
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  공급자별로 즉시 fetch 후 health_data 적재를 검증합니다.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                {providers.map((providerId) => (
                  ...
                  >
                    <RefreshCw
                      className={`h-4 w-4 ${testingProviderId === providerId ? "animate-spin" : ""}`}
                    />
                    {testingProviderId === providerId
                      ? "테스트 중..."
                      : `${getProviderMeta(providerId).label} 테스트`}
                  </Button>
                ))}
              </CardContent>
            </Card>
            {providerValidationSummary ? (
              <Card>
                <CardHeader>
                  <CardTitle>마지막 검증 결과</CardTitle>
                  <CardDescription>
                    {formatDateTime(providerValidationSummary.checkedAt)} ·{" "}
                    {providerValidationSummary.mode === "live-test"
                      ? "실데이터 테스트"
                      : "기간 적재"}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="font-medium">
                    {
                      getProviderMeta(providerValidationSummary.providerId)
                        .label
                    }
                  </div>
                  <div className="rounded-xl bg-muted p-3 text-sm">
                    {providerValidationSummary.message}
                  </div>
                  <div className="grid gap-3 md:grid-cols-3">
                    <div className="rounded-xl border p-3 text-sm">
                      <div className="text-muted-foreground">성공</div>
                      <div className="mt-1 font-semibold">
                        {providerValidationSummary.successCount ?? 0}
                      </div>
                    </div>
                    <div className="rounded-xl border p-3 text-sm">
                      <div className="text-muted-foreground">건너뜀</div>
                      <div className="mt-1 font-semibold">
                        {providerValidationSummary.skippedCount ?? 0}
                      </div>
                    </div>
                    <div className="rounded-xl border p-3 text-sm">
                      <div className="text-muted-foreground">실패 날짜</div>
                      <div className="mt-1 font-semibold">
                        {providerValidationSummary.failedDates?.length ?? 0}
                      </div>
                    </div>
                  </div>
                  ...
                  providerValidationSummary.failedDates.length > 0 ? (
                    <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
                      <div className="mb-1 font-medium">확인할 실패 항목</div>
                      <ul className="space-y-1">
                        {providerValidationSummary.failedDates
                          .slice(0, 6)
                          .map((item) => (
            ...
            <Card>
              ...
              <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {backgroundOptions.map((option) => (
                  ...
                  >
                    <div
                      className="mb-3 h-14 rounded-xl border"
          ...
          <TabsContent value="records" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  기록 파일을 읽어 공식 시간을 저장하고 프로필에 표시할 기록을
                  선택합니다.
                </CardDescription>
              </CardHeader>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      value={recordType}
                      onChange={(e) =>
                        setRecordTypeState(e.target.value as RecordType)
                      }
                    >
                    ...
                    <div key={record.id} className="rounded-xl border p-4">
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {record.type.toUpperCase()} · {record.officialTime}{" "}
                            · {record.certified ? "인증 완료" : "미인증"}
                          </div>
                        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  />
                  <Input
                    value={equipmentDistanceKm}
                    onChange={(e) => setEquipmentDistanceKm(e.target.value)}
                    placeholder="누적 거리 km"
                  />
                </div>
          ...
          <TabsContent value="data" className="space-y-6">
                  ...
                  <div key={section} className="space-y-3">
                    ...
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                      {displayOptions[section].map((option) => (
                        ...
                        >
                          <Checkbox
                            checked={displaySettings[section].includes(
                              option.key,
                            )}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    로컬 데이터와 서버 데이터를 개별적으로 관리할 수 있습니다.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3">
                  ...
                  <Button
                    variant="destructive"
                    onClick={() => void handleServerDelete()}
                  >
                    서버 데이터 삭제
                  </Button>
                  <Button onClick={() => void handleDeletionRequest()}>
                    서버 삭제 요청 제출
                  </Button>
                </CardContent>
              </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  내보내기, 삭제, 요청 제출 같은 민감 작업 이력을 표시합니다.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
